...
**Block:** A of 7 (v3) · **Date:** 2026-09-19 · **Status:** ⏳ unit tests green locally, browser tests running on CI
...
**Not yet mutation-checked:** removing `<ThemeEffect />` from the layout. The unit tests can't see
that, because they render `ThemeEffect` directly. Only V3A-10, a browser test, loads the real
layout. Browser tests run only on CI, so I'm pushing a throwaway branch with that one line removed,
to watch V3A-10 go red there.
...
| Suite | Count | Result |
|---|---|---|
| Unit (Vitest) | 259 | ✅ locally |
| Browser (Playwright), 79 tests × 2 devices | 158 runs | ⏳ |
| Database | 30 | ⏳ |
